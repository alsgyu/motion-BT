...
class K1IsaacLabKickPolicy(Policy):
    ...
    def _reset_ball_and_target(self) -> None:
        ...
        # The shared legacy MJCF contains an older, smaller ball. Override its
        # mutable compiled properties so this task matches the Size-4 training
        # ball without changing the legacy deployment task.
    ...
    def _ball_relative_xy(self, yaw: torch.Tensor) -> torch.Tensor:
        if self._ball_body_id is None:
            raise RuntimeError("Ball state was not initialized.")
